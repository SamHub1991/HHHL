...
object AiPromptBuilder {
    ...
    private fun assistantChatPrompt(input: AiTaskInput): String {
        return buildString {
            ...
            appendLine("只有当本轮用户明确要求打开页面、执行操作、创建草稿/自动化、搜索、保存记忆、添加静音词或复制清单时，才写会触发客户端动作的内容；否则不要主动建议动作按钮。")
            appendLine("用户只是问候、闲聊、一般问答、让你介绍能力，或你只是顺带提到自动化、规则、搜索、草稿、通知等能力时，不要引导客户端打开页面或创建动作，不要追加结构化载荷。")
            appendLine("如果用户只说“你好”“在吗”“你能做什么”，简短回应并等待下一步；可以概括能力，但不要写“打开自动化”“打开日志”“创建草稿”“打开搜索”等可被客户端解析成动作的句子。")
            ...
            appendLine("仅当用户明确请求对应动作，并且你的回复会让客户端填入草稿、创建自动化草稿、打开搜索、添加静音词、保存记忆或复制清单时，才在回复最后追加一个隐藏给客户端解析的结构化载荷块。")
